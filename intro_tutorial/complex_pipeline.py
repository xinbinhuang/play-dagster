...
def sort_by_calories(_, cereals):
    sorted_cereals = list(sorted(cereals, key=lambda cereal: int(cereal["calories"])))
...
def sort_by_protein(_, cereals):
    sorted_cereals = list(sorted(cereals, key=lambda cereal: int(cereal["protein"])))
...
def display_results(context, calorie_results, protein_results):
    context.log.info(
        "Least caloric cereal: {least_caloric}".format(least_caloric=calorie_results[0])
    )
    context.log.info(
        "Most caloric cereal: {most_caloric}".format(most_caloric=calorie_results[-1])
    )
    context.log.info(
